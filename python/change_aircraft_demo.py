...
def change_aircraft(flight_trip_id):
    ...
    if ft_table[0]: # checks that there is an item in ft_table
        cursor.execute(f"UPDATE FlightTrip SET Aircraft_id = NULL, DepartureTime = NULL, ArrivalTime = NULL WHERE FlightTrip_id = '{flight_trip_id}'")
    else :
...
def change_aircraft(flight_trip_id):
    ...
    aircraft_table = cursor.execute(f"SELECT * FROM Aircraft WHERE OnFlight = 0, AssignedToFlight = 0, OnLocation = 1").fetchall()
    ...
    new_aircraft_id = int(input("Please enter the aircraft ID of the aircraft you would like to assign to this flight trip: "))
    ...
    for row in aircraft_table: # aircraft_table is a list of tuples. for loop iterates over every tuple ie row
        if row[0] == new_aircraft_id: # aircraft_id will is at index 0 of each tuple
            ...
            cursor.execute(f"UPDATE FlightTrip SET Aircraft_id = {new_aircraft_id} WHERE FlightTrip_id = '{flight_trip_id}'") # sets aircraft id to the new one
            return new_aircraft_id
        elif row == aircraft_table[-1]: # if for loop reaches the last row without having found a match of the new aircraft id input by the user, user is asked to choose an aircraft that's available
    ...
    if ft_table[0]:
        for row in aircraft_table:
            if row[0] == new_aircraft_id:
                cursor.execute(f"UPDATE FlightTrip SET Aircraft_id = {new_aircraft_id}, DepartureTime = {departure_time}, ArrivalTime = {arrival_time} WHERE FlightTrip_id = '{flight_trip_id}'")
                return new_aircraft_id
